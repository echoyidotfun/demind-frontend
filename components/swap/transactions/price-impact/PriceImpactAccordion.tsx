...
import { getPriceImpactExceedsLabel } from "@/lib/modules/price-impact/price-impact.utils";
...
export function PriceImpactAccordion({
  ...
  const isUnknownPriceImpact =
    cannotCalculatePriceImpact || priceImpactLevel === "unknown";

  useEffect(() => {
    if (
      (hasToAcceptHighPriceImpact || isUnknownPriceImpact) &&
      !acceptPriceImpactRisk
    ) {
                ...
                <Box ml="md">
                  <AlertTitle>
                    {isUnknownPriceImpact
                      ...
                      : `Price impact is high: Exceeds ${getPriceImpactExceedsLabel(
                          priceImpactLevel
                        )}`}
                  </AlertTitle>
                  <AlertDescription>
              ...
              <CardBody>
                ...
                {isUnknownPriceImpact ? (
                  <Text color="grayText" fontSize="sm">
                    I accept that the price impact of this transaction is
                    unknown. I understand that proceeding may result in losses
                    if my transaction moves the market price unfavorably based
                    on the current depth of the market.
                  </Text>
                ) : (
                  <Text color="grayText" fontSize="sm">
                    I accept the high price impact of{" "}
                    {priceImpact && fNum("priceImpact", priceImpact)}. I
                    understand that this may result in losses, since the size of
                    my swap is likely to move the market price unfavorably based
                    on the current depth of the market.
                  </Text>
                )}
              </CardBody>
              <CardFooter pt="md">
                {!acceptPriceImpactRisk ? (
                  <Button onClick={handleClick} variant="secondary" w="full">
                    I accept {isUnknownPriceImpact ? "unknown" : "high"} price
                    impact
                  </Button>
                ) : (
                  <Button isDisabled variant="secondary" w="full">
                    {isUnknownPriceImpact ? "Unknown" : "High"} price impact
                    accepted
                  </Button>
                )}
              </CardFooter>
