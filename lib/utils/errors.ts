import { captureException, ScopeContext } from "@sentry/core";
import { TransactionConfig } from "../modules/web3/contracts/contract.types";
import { Hex, Address } from "viem";
import { buildTenderlyUrl } from "../modules/web3/useTenderly";
...
export function ensureError(
  value: unknown
): Error & { shortMessage?: string; digest?: string } {
...
export const swapApolloNetworkErrorMessage =
  "Apollo network error in DefaultSwapHandler";
...
function parseRpcRequestFailedError(
  error: Error,
  chainId: number
): TransactionConfig | undefined {
...
function parseRawCallArgumentsError(
  error: Error,
  chainId: number
): TransactionConfig | undefined {
