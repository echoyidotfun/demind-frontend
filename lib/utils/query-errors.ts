 ...
 */
export function sentryMetaForWagmiSimulation(
  errorMessage: string,
  extra: Extras
) {
 ...
 */
export function sentryMetaForWagmiExecution(
  errorMessage: string,
  extra: Extras
) {
...
export function captureWagmiExecutionError(
  error: unknown,
  errorMessage: string,
  extra: Extras
) {
...
export function captureFatalError(
  ...
  captureSentryError(
    error,
    createFatalMetadata(errorName, errorMessage, extra)
  );
}
...
export function captureNonFatalError({
  ...
  captureSentryError(
    error,
    createNonFatalMetadata(errorName, errorMessage, extra)
  );
}
...
export function shouldIgnore(message: string, stackTrace = ""): boolean {
  ...
  */
  if (message.includes(`Cannot set properties of null (setting 'content')`))
    return true;
  ...
  */
  if (
    message.startsWith("Cannot redefine property:") &&
    stackTrace.includes("inject.bundle.js")
  ) {
    return true;
  }
  ...
  */
  ...
  if (
    ...
    isInvariantRatioPIErrorMessage(message)
  ) {
    return true;
  }
