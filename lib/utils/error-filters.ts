...
export function isUnbalancedAddErrorMessage(error: Error | null): boolean {
  ...
  const hasErrors = (errorString: string) =>
    error?.message.includes(errorString);

  return errorStrings.some(hasErrors);
}

export function isInvariantRatioSimulationErrorMessage(
  errorMessage?: string
): boolean {
  return (
...
export function isInvariantRatioAboveMaxSimulationErrorMessage(
  errorMessage?: string
): boolean {
  return !!errorMessage?.includes("InvariantRatioAboveMax");
}
export function isInvariantRatioAboveMinSimulationErrorMessage(
  errorMessage?: string
): boolean {
