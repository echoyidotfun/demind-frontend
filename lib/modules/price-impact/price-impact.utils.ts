import { bn, fNum, isNegative, isZero } from "@/lib//utils/numbers";
...
export const getPriceImpactExceedsLabel = (
...
export function getPriceImpactLabel(
...
export function getFullPriceImpactLabel(
  ...
  if (isZero(priceImpact) || isNegative(priceImpact))
    return `${currencyPriceImpact} (0.00%)`;

  return `-${currencyPriceImpact}${getPriceImpactLabel(priceImpact)}`;
}

export function getMaxSlippageLabel(
  slippage: string | 0,
  currencyMaxSlippage: string
) {
  if (!slippage) return "-";
  if (isZero(slippage) || isNegative(slippage))
    return `${currencyMaxSlippage} (0.00%)`;
