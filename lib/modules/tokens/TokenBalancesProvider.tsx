...
export const TokenBalancesContext =
  createContext<UseTokenBalancesResponse | null>(null);
...
export function _useTokenBalances(
  ...
  if (!initTokens && !extTokens)
    throw new Error("initTokens or tokens must be provided");
  if (initTokens && extTokens)
  ...
  return {
    ...
    isBalancesRefetching: isRefetchingQueries(
      tokenBalancesQuery,
      nativeBalanceQuery
    ),
    setTokens,
...
export function TokenBalancesProvider({
  ...
  return (
    <TokenBalancesContext.Provider value={hook}>
      {children}
