...
export function useTransactionSteps(
  ...
  const [onSuccessCalled, setOnSuccessCalled] = useState<{
  ...
  const currentTransaction = currentStep
    ? getTransaction(currentStep.id)
    : undefined;
  ...
  const lastTransactionConfirmed =
    lastTransactionState === TransactionState.Completed;
  ...
  useEffect(() => {
    ...
    if (
      !isOnSuccessCalled(currentStep.id) &&
      currentTransaction?.result.isSuccess
    ) {
