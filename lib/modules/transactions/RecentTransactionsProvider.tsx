...
export type RecentTransactionsResponse = ReturnType<
  typeof _useRecentTransactions
>;
export const TransactionsContext =
  createContext<RecentTransactionsResponse | null>(null);
...
const TransactionStatusToastStatusMapping: Record<
  TransactionStatus,
  AlertStatus
> = {
  confirmed: "success",
...
export function _useRecentTransactions() {
  const [transactions, setTransactions] = useState<
    Record<string, TrackedTransaction>
  >({});
        ...
        try {
          ...
          */
          ...
          const isTimeoutError =
            ensureError(error).name === "WaitForTransactionReceiptTimeoutError";
          updatePayload[tx.hash] = {
            ...tx,
  ...
  function handleTransactionAdded(trackedTransaction: TrackedTransaction) {
    ...
    if (!trackedTransaction.hash) {
      throw new Error(
        "Attempted to add a transaction to the cache without a hash."
      );
    }
  ...
  function updateTrackedTransaction(
    hash: Hash,
    updatePayload: UpdateTrackedTransaction
  ) {
    // attempt to find this transaction in the cache
    const cachedTransaction = transactions[hash];

    if (!cachedTransaction) {
      console.log("Cannot update a cached tracked transaction", {
        hash,
        transactions,
      });
      throw new Error(
        "Cannot update a cached tracked transaction that does not exist."
      );
    }
  ...
  function updateLocalStorage(
    customUpdate?: Record<string, TrackedTransaction>
  ) {
...
export function RecentTransactionsProvider({
  children,
}: {
  children: ReactNode;
}) {
