...
import { getWrapType } from "../wrap.helper";
...
export class NativeWrapHandler implements SwapHandler {
  ...
  async simulate({
    ...variables
  }: SimulateSwapInputs): Promise<SimulateSwapResponse> {
    return {
  ...
  build({
    ...
    const value =
      wrapType === OWrapType.WRAP ? tokenIn.scaledAmount : BigInt(0);
