...
import { isDev } from "@/lib/configs/app.config";
import {
  captureError,
  getTenderlyUrlFromErrorMessage,
  ...
  queryCache: new QueryCache({
    ...
    onError: (error, query) => {
      ...
      if (sentryContext?.extra && !getTenderlyUrl(sentryMeta)) {
        sentryContext.extra.tenderlyUrl = getTenderlyUrlFromErrorMessage(
          error,
          sentryMeta
        );
      }
...
export function ReactQueryClientProvider({
  ...
  return (
    <QueryClientProvider client={queryClient}>
      {children}
      {isDev && shouldShowReactQueryDevtools && (
 ...
 */
function shouldIgnoreEdgeCaseError(
  error: Error,
  sentryMeta: SentryMetadata
): boolean {
  return false;
}
